...
nutzungsbereich = st.sidebar.selectbox("Analyse-Bereich wählen", ["Tagesanalyse", "Wochenanalyse", "Monatsanalyse"])
...
pv_csvs = st.sidebar.file_uploader("2️⃣ CSV PV-Erzeugung (datetime, power_kw)", type="csv", accept_multiple_files=True)
...
# Hilfsfunktion: kW -> kWh (15min) Fest
INTERVALL_H = 0.25
...
def main():
    ...
    # Merge Netz & PV
    df_all = pd.merge(df_pv[['datetime','PV_gesamt_kWh']], df_netz, on='datetime', how='outer').fillna(0)
    ...
    iqr = q3 - q1
    outliers = df_all[(df_all['PV_gesamt_kWh'] < q1 - 1.5*iqr) | (df_all['PV_gesamt_kWh'] > q3 + 1.5*iqr)]
    ...
    st.metric("Performance Ratio (PR)", f"{pr_ratio:.1f} %")
